...
class Kalendar(ctk.CTkFrame):
    ...
    def create_widgets(self):
        self.label = ctk.CTkLabel(self, text="Calendar", font=ctk.CTkFont(size=50, weight="bold"))
    ...
    def populate_dates(self):
        ...
        if not self.date_buttons_created:
            ...
            for week_num in range(6):  # Maximum 6 weeks in a month view
                week_buttons = []
                for day_num in range(7):  # 7 das per week
                    date_button = ctk.CTkButton(
                        ...
                        fg_color="#2b2b2b",
                        hover_color="#3b3b3b",
                        command=lambda r=week_num, c=day_num: self.button_clicked(r, c)
                    )
                    date_button.grid(row=week_num + 1, column=day_num, padx=5, pady=5, sticky="nsew")
                    week_buttons.append(date_button)
        ...
        for week_num, week in enumerate(month_calendar):
            for day_num, date in enumerate(week):
                ...
                if date == 0:
                    button.configure(text="", state="disabled", hover_color="#2b2b2b")
                else:
                    button.configure(text=str(date), state="normal", hover_color="#3b3b3b")
    ...
    def save_todo(self):    
        if self.to_do_frame is None or not hasattr(self, 'textbox'):
